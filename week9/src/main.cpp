...
public:
	...
	void setPhongUniforms(std::shared_ptr<Program> prog)
	{
	...
	void render()
	{
		...
		M->pushMatrix();
			...
			M->pushMatrix();
				...
				setPhongUniforms(meshProg);

				for (int i = 0; i < shape_list.size(); i++)
				{
					shape_list[i]->draw(meshProg);
				}
		...
		//RAINDBOW BOIS
		//CHECKED_GL_CALL(glUniform4f(partProg->getAttribute("pColor"), getRand(), getRand(), getRand(), 1));
